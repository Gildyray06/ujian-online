...
class soal_ujian extends CI_Controller {
	...
	{	
		if (isset($_GET['id'])) {
			...
			$data['soal_ujian'] = $this->db->query('SELECT * from tb_soal_ujian  where  tb_matapelajaran.id_matapelajaran="'.$id.'" order by id_soal_ujian desc')->result();
			$data['kelas']=$this->m_data->get_data('tb_matapelajaran')->result();
		} else {
			$data['soal_ujian'] = $this->db->query('SELECT * FROM tb_soal_ujian order by id_soal_ujian desc')->result();
			$data['kelas']=$this->m_data->get_data('tb_matapelajaran')->result();
		}					
